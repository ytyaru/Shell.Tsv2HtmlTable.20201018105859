...
set -Ceu
#---------------------------------------------------------------------------
# TSVをHTMLのtableタグに変換する。行ヘッダ部分のみ。
...
#	. $HERE/lib/enclose.sh
	...
	# 小A	小B	小C	小D		小E
	OutputThRows() {
		echo -e "$TSV" | while IFS= read line; do { Enclose 'tr' "$(echo -e "$(OutputThRow "$line")" | Encloses 'th')"; } done;
	}
	OutputThRow() {
		paste \
			<(echo -e "$1" | tr '\t' '\n') \
			<(echo -e "$1" | echo -e "$(ColspanLength)" | echo -e "$(ColspanAttr)")
			<(echo -e "$1" | echo -e "$(RowspanLength)" | echo -e "$(RowspanAttr)")
	}
	# rowspan:
